...
# Beat/downbeat activation thresholds the particle filter requires before
# committing a beat. Upstream defaults are 0.4 — tuned for benchmark
# precision. For live lighting we prefer higher recall, so we lower these
# to 0.25. Exposed as module attributes so the adapter can override at
# runtime from settings without re-vendoring the file.
BEAT_ACTIVATION_THRESHOLD = 0.25
DOWNBEAT_ACTIVATION_THRESHOLD = 0.25
    ...
    def process(self, activations):
        ...
        for i in range(len(activations)):  # loop through the provided frame/s to infer beats/downbeats
            ...
            if ((gathering - self.beat[self.st.state_intervals[self.beat] == self.st.state_intervals[gathering]]) < (
                ...
                if self.down_max in self.st2.first_states[0] and self.path[-1][1] !=1 and both_activations[i][1]>DOWNBEAT_ACTIVATION_THRESHOLD:
                    self.path = np.append(self.path, [[self.offset + self.counter * self.T, 1]], axis=0)
                    if self.mode == 'stream' or self.mode == 'realtime':
                        print("*beat!")
                elif (activations[i]>BEAT_ACTIVATION_THRESHOLD) :
